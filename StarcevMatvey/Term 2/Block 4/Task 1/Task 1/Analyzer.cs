        ...
        public void Analyze()
        {
            ...
            for (var i = 0; i < data.Count; i++)
            {
                if (data[i].type == TypeOfParsedStr.Argument)
                {
                    ...
                    else if (!lastIsCommand)
                    {
                        var lastArg = arguments.Dequeue();
                        lastArg += "\n" + data[i].str;
                        arguments.Enqueue(lastArg);
                ...
                else if (data[i].type == TypeOfParsedStr.ProcessStart)
                {
                    arguments.Enqueue(data[i].str);
                    listOfCmds.Add(Command.SetArgument);
                    listOfCmds.Add(Command.ProcessStart);
