...
void* my_malloc(size_t size)
{
	...
	while (current->next_block_offset <= MEM_SIZE && current->next_block_offset >= 0)
	{
		...
		if (current->in_use == 0)
		{
			...
			if (current->next_block_offset - (int)current + (int)memory_begin >= size + 2 * sizeof(*current))
			{
				// Creating new block
...
void merge()
{
	...
	while (current->next_block_offset != 0 && current->next_block_offset + sizeof(*current) <= MEM_SIZE)
	{
		if (current->next_block_offset >= MEM_SIZE)
			current->next_block_offset = 0;
...
void* my_realloc(void* ptr, size_t size)
{
	...
	int block_size = ((struct memory_block*)ptr - 1)->next_block_offset + (int)ptr - (int)memory_begin;
