...
namespace TomorrowIO
{
    ...
    {
        public TomorrowIOParser(string token)
        {
            if (token == "") token += "null";
            key = token;
            parsingParams = "temperature,cloudCover,humidity,precipitationType,precipitationIntensity,windSpeed,windDirection".Split(",");
            Link = $"https://api.tomorrow.io/v4/timelines?location=59.873703,29.828038&fields={string.Join(",", parsingParams)}&timesteps=current&units=metric&apikey={key}";
            Headers = null;
