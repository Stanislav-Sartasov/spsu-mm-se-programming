...
internal class MartingaleBetStrategyTest {
	private val initialBet = 10.0
	private val strategy = MartingaleBetStrategy(initialBet)
	private val account = CashAccount(100.0)

	@Test
	fun `nextBet() should return bet with doubled amount and same bet if last game is lost`() {
	...
	@Test
	fun `nextBet() should return bet with initial amount if last game is won`() {
		val (bet1, amount1) = strategy.nextBet()
