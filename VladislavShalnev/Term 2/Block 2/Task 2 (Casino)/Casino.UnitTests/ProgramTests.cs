...
public class ProgramTests
{
	...
	public void SubscribeToEventsTest()
	{
		...
		foreach (Player bot in game.Players)
		{
			...
			Assert.IsNull(bot.OnBlackjack);
			Assert.IsNull(bot.OnKick);
			Assert.IsNull(bot.OnBet);
		}

		Assert.IsNull(game.Dealer.OnBlackjack);
		...
		foreach (Player bot in game.Players)
		{
			...
			Assert.IsNotNull(bot.OnBlackjack);
			Assert.IsNotNull(bot.OnKick);
			Assert.IsNotNull(bot.OnBet);
		}

		Assert.IsNotNull(game.Dealer.OnBlackjack);
		...
		foreach (Player bot in game.Players)
		{
			...
			bot.OnHit?.Invoke();
			bot.OnStand?.Invoke();
			bot.OnBlackjack?.Invoke();
			bot.OnKick?.Invoke();
			bot.OnBet?.Invoke();
		}
		
		game.Dealer.OnBlackjack?.Invoke();
		game.OnStart?.Invoke();
		game.OnEnd?.Invoke();
	}
}