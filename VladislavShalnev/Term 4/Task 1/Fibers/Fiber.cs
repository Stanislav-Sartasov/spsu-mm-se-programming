...
namespace Fibers
{
	public class Fiber
	{
		...
		// Protecting the runner from the garbage collector
		// https://stackoverflow.com/questions/4855513/callbackoncollecteddelegate-was-detected
		private UnmanagedFiberAPI.LPFIBER_START_ROUTINE _runnerReference;
		...
		public Fiber(Action action, int duration, int priority)
		{
			InnerCreate(action);
			Priority = priority;
		...
		private void InnerCreate(Action action)
		{
			...
			_runnerReference = FiberRunnerProc;
			UnmanagedFiberAPI.LPFIBER_START_ROUTINE lpFiber = _runnerReference;
