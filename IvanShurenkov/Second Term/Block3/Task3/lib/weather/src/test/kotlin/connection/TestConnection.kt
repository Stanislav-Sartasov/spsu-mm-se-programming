...
class TestConnection {
    ...
    fun `Test error code in requestGet`() {
        ...
        val testStream = DI {
            bindSingleton<ErrorStream>() { mockk<ErrorStream>() }
        }

        val err: ErrorStream by testStream.instance()
        every { err.print(any()) } answers { nothing }

        val connection = Connection(conn, testStream)
        assertEquals("$responseCode", connection.requestGet())
        verify { err.print("Response code: $responseCode") }
