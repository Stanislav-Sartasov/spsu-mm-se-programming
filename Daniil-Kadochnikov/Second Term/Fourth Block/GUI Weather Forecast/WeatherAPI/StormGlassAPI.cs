	...
	public class StormGlassAPI : AWeatherAPI
	{
		...
		public override async Task<string> GetDataAsync()
		{
			try
			{
				...
				HttpResponseMessage responseHttp = await Client.GetAsync(URL + $"&start={new DateTimeOffset(DateTime.UtcNow).ToUnixTimeSeconds()}&end={new DateTimeOffset(DateTime.UtcNow).ToUnixTimeSeconds()}&source=sg");
