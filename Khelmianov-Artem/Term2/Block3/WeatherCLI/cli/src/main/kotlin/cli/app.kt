...
class App {
    var location = Coordinates(59.9F, 30.3F)
        private set
    private var apis = mutableListOf<IWeatherAPI>()
    private lateinit var currentApi: IWeatherAPI
    private var data = WeatherData()
    private val client = HttpClient() {
        expectSuccess = true        // todo: check status code instead?
        install(HttpTimeout) {
    ...
    fun addAPI(api: List<IWeatherAPI>) = apis.addAll(api)
