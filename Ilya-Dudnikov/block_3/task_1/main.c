...
int main(int argc, char *argv[]) {
	...
	char **string_arr = (char **)malloc(word_count * sizeof(char *));
	for (int i = 0; i < word_count; i++)
	{
		string_arr[i] = (char *)malloc(longest_word * sizeof(char));
	}
	parse(from, size, string_arr);

	for (int i = 0; i < word_count; i++)
	{
		for (int j = 0; j < longest_word && string_arr[i][j] != '\0'; j++)
		{
...
char *get_input(char *filename, int oflag, int mode, int *needed_size)
{
	...
	if (oflag & O_RDONLY)
		return mmap(0, size, PROT_READ, MAP_PRIVATE, file_desc, 0);
	else
...
{
	for (int i = 0, row = 0, col = 0; i < size; i++, col++)
	{
		if (mem_file[i] == '\n')
		{
			col = 0;
